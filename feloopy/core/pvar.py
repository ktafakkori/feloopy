...
def add_gekko_pvar(modelobject, var_name, b, dim=0):
    if dim == 0:
        ...
        if len(dim) == 1:
            return {key: modelobject.Var(lb=b[0], ub=b[1], integer=False) for key in dim[0]}
        else:
            return {key: modelobject.Var(lb=b[0], ub=b[1], integer=False) for key in it.product(*dim)}
...
def add_picos_pvar(modelobject, var_name, b, dim=0):
    if dim == 0:
...
def add_cplex_pvar(modelobject, var_name, b, dim=0):
    if dim == 0:
...
def add_gurobi_pvar(modelobject, var_name, b, dim=0):
    if dim == 0:
...
def add_xpress_pvar(modelobject, var_name, b, dim=0):
    if b[0] == None:
        b[0] = 0
    if b[1] == None:
...
def add_linopy_pvar(modelobject, var_name, b, dim=0):
    if dim == 0:
...
def add_cvxpy_pvar(modelobject, var_name, b, dim=0):
    if dim == 0:
...
def add_cylp_pvar(modelobject, var_name, b, dim=0):
    if dim == 0:
...
def add_mip_pvar(modelobject, var_name, b, dim=0):
    if dim == 0:
...
pvar_maker = {
    ...
    "picos": add_picos_pvar,
    "cplex": add_cplex_pvar,
    "gurobi": add_gurobi_pvar,
    "xpress": add_xpress_pvar,
    "linopy": add_linopy_pvar,
    "cvxpy": add_cvxpy_pvar,
    "cylp": add_cylp_pvar,
    "mip": add_mip_pvar,
    "ga": add_ga_pvar
}
